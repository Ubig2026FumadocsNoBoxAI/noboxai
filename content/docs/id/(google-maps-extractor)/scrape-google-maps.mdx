...
Penghapusan data akun bersifat permanen dan tidak dapat dibatalkan. Pastikan data yang akan dihapus benar-benar tidak diperlukan lagi.
</Callout>
...
Beberapa tips untuk memaksimalkan hasil scraping Google Maps:
 
- Tentukan Limit Result yang realistis sesuai dengan kebutuhan distribusi data Anda.
- Manfaatkan filter jam buka dan rating untuk mendapatkan data prospek bisnis yang lebih berkualitas.
- Gunakan penamaan list (Save to List) yang spesifik (misal: "Apotek - Jakarta Selatan") agar data mudah dikelola.
- Aktifkan fitur Load Images jika Anda membutuhkan data visual lokasi bisnis untuk keperluan verifikasi.
- Lakukan pembersihan log secara berkala agar performa aplikasi NoBox Desktop tetap optimal.
...
Jika mengalami kendala saat melakukan scraping, lakukan langkah-langkah berikut:
 
- Jika jumlah data yang didapat lebih sedikit dari limit, kemungkinan terdapat data duplikat yang secara otomatis disaring oleh sistem.
- Pastikan lokasi pada Google Maps sudah mengarah ke area yang benar sebelum menekan tombol **[Start]**.
- Jika aplikasi terasa lambat, coba kurangi jumlah limit result atau matikan opsi Load Images.
- Periksa status akun Anda di website NoBox.Ai jika data gagal tersinkronisasi.
- Restart aplikasi NoBox Desktop jika terjadi pembekuan (freezing) saat proses pencarian berlangsung.
