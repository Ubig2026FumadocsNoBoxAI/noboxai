---
title: Google Maps Extractor
description: Mengenal struktur dasbor, kanban, list, dan formulir di NoBox.AI
icon: MapPin
---

import { ImageZoom } from "fumadocs-ui/components/image-zoom";

Untuk mendukung semua kelebihannya, Google Maps Extractor menawarkan sejumlah fitur menarik yang dapat Anda manfaatkan. Diantara sekian banyaknya pilihan fitur yang ada, Anda bisa memanfaatkan beberapa fitur pilihan di bawah ini:

<iframe
  width="800"
  height="450"
  className="rounded-lg"
  src="https://www.youtube.com/embed/i2JsGX__WRg/"
  title="01. Instalasi NoBox Desktop"
  frameborder="0"
  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
  referrerpolicy="strict-origin-when-cross-origin"
  allowfullscreen
></iframe>

## **Set Karakter Unicode**

NoBox dapat menyimpan hasil pencarian yang diambil dalam format unicode seperti Arabic, Chinesse, Japan, dl&#x6C;**.**

<ImageZoom
  src="/images/id/Unicode.png"
  alt="Tampilan Dasbor NoBox"
  width={800}
  height={450}
  className="rounded-lg border shadow-sm mb-4"
/>

- Ubah setting bahasa terlebih dahulu.
- Lakukan ekstraksi data.&#x20;
- Jika sudah selesai nantinya data hasil ekstraksi akan tersimpan sesuai bahasa yang dipilih.

## **Interval Get List**

Fitur untuk mengatur waktu cepat lambatnya antara permintaan satu dengan berikutnya, agar mensimulasikan seperti manusia nyata yang menjalankan browser.

<ImageZoom
  src="/images/id/Interval%20(1).png"
  alt="Tampilan Dasbor NoBox"
  width={800}
  height={450}
  className="rounded-lg border shadow-sm mb-4"
/>

## **Page Process**

Halaman yang berfungsi untuk setting ketentuan data yang akan diambil.

<ImageZoom
  src="/images/id/Page%20Process.png"
  alt="Tampilan Dasbor NoBox"
  width={800}
  height={450}
  className="rounded-lg border shadow-sm mb-4"
/>

<table>
  <thead>
    <tr>
      <th width="174.60003662109375">Fitur</th>
      <th>Uraian</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>Limit Result</td>
      <td>
        Penambahan data sesuai dengan kebutuhan berapa jumlah data yang di
        inginkan.
      </td>
    </tr>
    <tr>
      <td>Save To List</td>
      <td>
        Simpan dan tambah nama file, jadi ketika ingin menambah data list bisa
        menggunakan fitur ini, caranya tinggal menekan tombol{" "}
        <strong>[+]</strong> lalu beri nama list sesuai yang di inginkan setelah
        itu tekan <strong>[OK]</strong>
      </td>
    </tr>
    <tr>
      <td>Option Debug </td>
      <td>Munculkan logs aktivitas dengan sangat detail</td>
    </tr>
    <tr>
      <td>Option Load Images</td>
      <td>Menampilkan gambar pada data</td>
    </tr>
    <tr>
      <td>Other Option</td>
      <td>
        Pengaturan Interval untuk pemilihan lokasi negara, dan di bagian bawah
        terdapat pengaturan Interval untuk pengambilan data.{" "}
      </td>
    </tr>
  </tbody>
</table>

## **Page Result**

Result adalah halaman yang menampilkan hasil dari data yang diambil.

<ImageZoom
  src="/images/id/Page%20Result.png"
  alt="Tampilan Dasbor NoBox"
  width={800}
  height={450}
  className="rounded-lg border shadow-sm mb-4"
/>

## **Page Logs**

Logs adalah halaman untuk melihat semua aktivitas sistem yang sedang berlangsung.

<ImageZoom
  src="/images/id/Page%20Logs.png"
  alt="Tampilan Dasbor NoBox"
  width={800}
  height={450}
  className="rounded-lg border shadow-sm mb-4"
/>

## **Multi Tab**

Digunakan untuk menjalankan proses pencarian dengan berbagai kata kunci secara bersamaan.

<ImageZoom
  src="/images/id/Multi%20Tab.png"
  alt="Tampilan Dasbor NoBox"
  width={800}
  height={450}
  className="rounded-lg border shadow-sm mb-4"
/>

- Lakukan ekstrasi data pada 2 akun extractor, tambahkan dengan **"Add Account"**
- Extractor yang pertama, misalnya lakukan pencarian bebas, dan dengan jumlah data sebanyak 5, dan tambahkan list yang baru.

## **Website NoBox.Ai**

Untuk urusan data yang sudah diekstraksi akan langsung tersimpan pada website NoBox.Ai, sehingga Anda dapat mengakses dan memantau hasilnya dengan browser dan perangkat apa saja.

<ImageZoom
  src="/images/id/Contacts%20(2).png"
  alt="Tampilan Dasbor NoBox"
  width={800}
  height={450}
  className="rounded-lg border shadow-sm mb-4"
/>

- Masuk ke bagian **[Contacts]** dan pilih menu **[Contacts]**
- Pilih filter **[Source -> Google Maps]**
- Aktifkan fitur **[Auto Refresh]**, untuk memantau data apa saja yang berhasil diproses

## **Export Data**

NoBox menyediakan opsi untuk menyimpan data yang diekstraksi dalam format Excel (.xlsx, .csv), dan file format JSON (.json).

<ImageZoom
  src="/images/id/Export.png"
  alt="Tampilan Dasbor NoBox"
  width={800}
  height={450}
  className="rounded-lg border shadow-sm mb-4"
/>

- Klik **[Export to Excel]** dibagian page **[Process]**&#x20;
- Untuk melihat data apa saja yang baru saja diambil, bisa juga dilihat dari page **[Results]** lalu klik button **[Action]** untuk export dalam format excel, csv, dan JSON

## **Penanganan Komputer Mati Saat Ekstrak Data**

NoBox menyediakan fitur jika komputer/software mati secara tidak terduga saat proses ekstraksi data.&#x20;

<ImageZoom
  src="/images/id/Komputer%20mati%20(1).png"
  alt="Tampilan Dasbor NoBox"
  width={800}
  height={450}
  className="rounded-lg border shadow-sm mb-4"
/>

- Buka kembali aplikasi NoBox dan masuk ke menu **[Extractor -> Google Maps]**. Akan ada pemberitahuan bahwa proses sebelumnya masih belum selesai.
- Klik **[OK]** untuk melanjutkan proses pencarian data
- Klik **[Cancel]** untuk membatalkan pencarian

---
 
<Tips>
## **Tips Penggunaan**
 
Agar proses ekstraksi data dari Google Maps lebih efektif, perhatikan beberapa tips berikut:
 
- Gunakan fitur Multi Tab untuk menjalankan beberapa pencarian dengan kata kunci berbeda secara bersamaan.
- Atur Interval Get List agar proses ekstraksi mensimulasikan perilaku manusia dan menghindari deteksi sistem.
- Pastikan setting Unicode sudah sesuai jika Anda mencari data di negara dengan karakter khusus seperti Arab atau Jepang.
- Gunakan fitur Auto Refresh pada website NoBox.Ai untuk memantau data yang masuk secara real-time.
- Ekspor data ke format Excel secara berkala untuk cadangan data lokal Anda.
 
</Tips>
 
---
 
<Kendala>
## **Penanganan Kendala**
 
Jika mengalami kendala saat menggunakan Google Maps Extractor, lakukan langkah berikut:
 
- Jika proses terhenti secara tiba-tiba, buka kembali aplikasi dan klik **[OK]** pada notifikasi untuk melanjutkan proses sebelumnya.
- Pastikan koneksi internet stabil; gangguan koneksi dapat mengakibatkan data gagal ditarik secara sempurna.
- Jika hasil ekstraksi sedikit, coba ubah kata kunci pencarian atau perluas area pencarian di peta.
- Periksa log aktivitas di halaman **[Logs]** untuk melihat detail galat (error) jika sistem tidak berjalan semestinya.
- Hubungi tim support jika masalah lisensi atau sinkronisasi ke website NoBox.Ai terjadi.
 
</Kendala>
 
---
 
Jika ada masalah atau kesulitan terkait NoBox Desktop, silahkan hubungi kami melalui [Support Ticket](https://crm.nobox.ai/clients/tickets)
