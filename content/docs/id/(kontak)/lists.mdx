---
title: List
description: Mengenal struktur dasbor, kanban, list, dan formulir di NoBox.AI
icon: List
---

import { ImageZoom } from "fumadocs-ui/components/image-zoom";

List adalah salah satu menu pada Nobox.Ai yang digunakan untuk mengelompokkan data kontak.

<iframe
  width="800"
  height="450"
  className="rounded-lg"
  src="https://www.youtube.com/embed/OfOOjWDD_1c"
  title="Pengenalan Tampilan NoBox"
  frameborder="0"
  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
  referrerpolicy="strict-origin-when-cross-origin"
  allowfullscreen
></iframe>

<ImageZoom
  src="/images/id/list.png"
  alt="Tampilan List NoBox"
  width={200}
  height={450}
  className="rounded-lg border my-8 mx-auto shadow-sm mb-4"
/>

## **Tambah List**

Untuk membuat data list, silahkan klik modul **[Kontak]**, lalu klik menu **[List]➔\[Tambah List]** maka akan tampil dialog **[Tambah List]** kemudian isi nama list yang akan dibuat.

<ImageZoom
  src="/images/id/Tambah%20List.png"
  alt="Tampilan List NoBox"
  width={800}
  height={450}
  className="rounded-lg border shadow-sm mb-4"
/>

Jika Anda sudah melengkapi isian yang ada, klik **[Simpan]** untuk menyimpan data list yang baru saja Anda buat.

## **Edit dan Hapus Data List**

Untuk mengedit suatu data list, silahkan pilih terlebih dahulu data mana yang akan diedit. Editlah data Anda dan klik **[Simpan]** untuk menyimpan perubahan tersebut.

Untuk menghapus suatu data list adalah dengan pilih data list yang akan dihapus, kemudian klik **[Hapus]**, maka akan tampil dialog konfirmasi apakah Anda ingin menghapus data. Jika ya, klik **[Ya]** jika tidak maka klik **[Tidak]**.

## **Tambah Kontak**

Untuk memudahkan menambahkan kontak pada list, Anda dapat menggunakan fitur berikut.

<ImageZoom
  src="/images/id/List%20Tambah%20Kontak.png"
  alt="Tampilan List NoBox"
  width={800}
  height={450}
  className="rounded-lg border shadow-sm mb-4"
/>

## **Tambah Dari Kontak**

Untuk menambahkan kontak baru melalui data kontak yaitu dengan cara pilih list yang akan digunakan, klik tombol **[Tambah Kontak]➔** klik tombol **[Tambah dari Kontak] ➔** pilih kontak **➔** klik **[OK]**.

<ImageZoom
  src="/images/id/tambah%20dari%20kontak.png"
  alt="Tampilan List NoBox"
  width={800}
  height={450}
  className="rounded-lg border shadow-sm mb-4"
/>

## **Tambah Dari Target Promosi**

Untuk menambahkan kontak melalui target promosi yaitu dengan cara pilih list yang akan digunakan, klik tombol **[Tambah Kontak]**➔ klik tombol **[Tambah dari Target Promosi] ➔** pilih data promosi **➔** klik **[OK]**.

<ImageZoom
  src="/images/id/Link.png"
  alt="Tampilan Dasbor NoBox"
  width={800}
  height={450}
  className="rounded-lg border shadow-sm mb-4"
/>

## **Hapus Semua Kontak**

Untuk menghapus seluruh data kontak yang ada di dalam suatu list yaitu dengan pilih data list yang diinginkan, kemudian klik **[Hapus Semua Kontak]**, maka akan tampil dialog konfirmasi apakah Anda ingin menghapus semua data kontak yang ada dilist tersebut. Jika ya, klik **[Ya]** jika tidak maka klik **[Tidak]**.

<ImageZoom
  src="/images/id/List%20Hapus%20Kontak.png"
  alt="Tampilan List NoBox"
  width={800}
  height={450}
  className="rounded-lg border shadow-sm mb-4"
/>

---
 
<Tips>
## **Tips Penggunaan**
 
Berikut beberapa tips untuk memaksimalkan penggunaan menu List:
 
- Gunakan List untuk mengategorikan kontak berdasarkan minat, lokasi, atau status pelanggan.
- Manfaatkan fitur 'Tambah dari Kontak' untuk memasukkan data yang sudah ada ke dalam list baru.
- Gunakan 'Tambah dari Target Promosi' untuk mengumpulkan kontak yang merespon kampanye tertentu.
- Lakukan pembersihan (Hapus Semua Kontak) jika list tersebut ingin digunakan kembali untuk kategori yang berbeda.
 
</Tips>
 
---
 
<Kendala>
## **Penanganan Kendala**
 
Jika mengalami kendala saat menggunakan menu List, lakukan langkah-langkah berikut:
 
- Refresh halaman jika jumlah kontak di dalam list tidak segera berubah setelah penambahan.
- Pastikan Anda memilih kontak yang benar saat menggunakan fitur 'Tambah dari Kontak'.
- Periksa kembali koneksi internet jika proses pemindahan kontak antar list terasa lambat.
- Hubungi administrator jika fitur 'Hapus Semua Kontak' tidak dapat dijalankan (terkait hak akses).
 
</Kendala>
 
---

Jika ada masalah atau kesulitan terkait Nobox.Ai, silahkan hubungi kami melalui [Support Ticket](https://crm.nobox.ai/clients/tickets)
